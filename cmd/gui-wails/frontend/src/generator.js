...
export function initGenerator(root, playback) {
  root.innerHTML = `
    ...
    <div class="checkbox-row"><input type="checkbox" id="gen-roi2-mode" /><label for="gen-roi2-mode">2. Region markieren (für Tf/Tj; alternativ Shift halten)</label></div>
    ...
    <div class="row" style="align-items:center;">
      ...
      <select id="gen-profile">
        ...
        <option value="tj">Tf / Tj (Abstand, Sog)</option>
      </select>
    </div>
    ...
    <p class="hint" id="gen-tj-hint" style="margin:0 0 10px 0; display:none;">Tf/Tj: zwei Regionen markieren
      (erste + zweite). Signal = Abstand. Auf dem Neo 2 steuert die Position den Sog,
      Vibration bleibt aus. Pos-Klammer 20–90.</p>
  ...
  let roi2 = null; // zweite Region für tf/tj
  let drawingRoi2 = false; // Shift oder Knopf „2. Region“
  ...
  function redraw() {
    ...
    if (dragging || roi) {
      ...
      ctx.strokeStyle = drawingRoi2 ? '#ff9f1a' : '#00c8ff';
      ctx.lineWidth = 2;
      ctx.strokeRect(dx0, dy0, dw, dh);
      ctx.fillStyle = drawingRoi2 ? 'rgba(255,159,26,0.15)' : 'rgba(0,200,255,0.15)';
      ctx.fillRect(dx0, dy0, dw, dh);
    }
    if (roi2 && !dragging) {
      const scaleX = canvas.width / nativeW, scaleY = canvas.height / nativeH;
      const dx0 = roi2.x * scaleX, dy0 = roi2.y * scaleY, dw = roi2.w * scaleX, dh = roi2.h * scaleY;
      ctx.strokeStyle = '#ff9f1a';
      ctx.lineWidth = 2;
      ctx.strokeRect(dx0, dy0, dw, dh);
      ctx.fillStyle = 'rgba(255,159,26,0.15)';
      ctx.fillRect(dx0, dy0, dw, dh);
    }
  }
  ...
  canvas.addEventListener('mousedown', e => {
    ...
    dragging = true;
    drawingRoi2 = e.shiftKey || (el('#gen-roi2-mode') && el('#gen-roi2-mode').checked);
  });
  ...
  window.addEventListener('mouseup', () => {
    ...
    if (drawingRoi2) {
      roi2 = box;
      el('#gen-roi-label').textContent = `1: ${roi ? `x=${roi.x} y=${roi.y} w=${roi.w} h=${roi.h}` : '—'} · 2: x=${roi2.x} y=${roi2.y} w=${roi2.w} h=${roi2.h}`;
    } else {
      roi = box;
      el('#gen-roi-label').textContent = `Region: x=${roi.x} y=${roi.y} w=${roi.w} h=${roi.h} (Videopixel)` +
        (roi2 ? ` · 2: x=${roi2.x} y=${roi2.y} w=${roi2.w} h=${roi2.h}` : '');
    }
  ...
  function isTjProfile() {
    const p = el('#gen-profile').value;
    return p === 'tj' || p === 'tf';
  }
  function updateGenerateEnabled() {
    const ok = videoPath && roi && (!isTjProfile() || roi2);
    el('#gen-generate').disabled = !ok;
    el('#gen-tj-hint').style.display = isTjProfile() ? 'block' : 'none';
  }
  el('#gen-profile').addEventListener('change', updateGenerateEnabled);

  async function generate() {
    if (!videoPath || !roi) return;
    if (isTjProfile() && !roi2) {
      alert('Tf/Tj braucht eine zweite Region (Shift halten beim Ziehen oder „2. Region“).');
      return;
    }
    ...
    if (roi2) {
      opts.x2 = roi2.x; opts.y2 = roi2.y; opts.w2 = roi2.w; opts.h2 = roi2.h;
    }
    GenerateScript(opts);
  }
