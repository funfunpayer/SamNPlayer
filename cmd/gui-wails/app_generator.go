...
// einer Goroutine; Ergebnis/Fehler kommen über das Event "generate:autoroi".
...
func (a *App) ScriptExistsForVideo(videoPath string) (bool, error) {
		...
		if !opts.Overwrite {
			if _, err := os.Stat(outPath); err == nil {
				runtime.EventsEmit(a.ctx, "generate:done", map[string]any{
					"error": "Zieldatei existiert bereits: " + outPath +
						" – Überschreiben nur mit explizitem overwrite=true",
				})
				return
			}
		}
