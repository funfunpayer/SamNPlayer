...
func TestBookmarksRoundTrip(t *testing.T) {
	...
	if len(out) != 2 || out[0].Name != "A" || out[1].Time != 500 {
		t.Fatalf("got %+v", out)
	}
	// creator preserved
	data, _ := os.ReadFile(path)
	if !containsBytes(data, []byte(`"creator":"t"`)) && !containsBytes(data, []byte(`"creator": "t"`)) {
		// compact marshal may keep creator
		raw := string(data)
		if !containsBytes(data, []byte(`creator`)) {
			t.Fatalf("metadata creator lost: %s", raw)
		}
	}
}
