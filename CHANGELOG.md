...
- **Generate → Advanced: "Record tip/partner trajectory" checkbox restored**
  — the MT-Seed rebase had silently dropped it (and the `captureTrajectory`
  payload field) from `generator.js`, making the MT-Debug trajectory feature
  unreachable from the GUI even though capture/overlay code stayed correct.
