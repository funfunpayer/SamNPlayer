...
<p align="center">
  ...
  ·
  <a href="docs/EMOTION_SCRIPT.md">Emotion Script</a>
  ·
...
**heavy Electron shell**. SamNPlayer is one native desktop **Emotion Generator** —
Create · Play · Train — and stays honest about what is measured vs. only suggested.
...
| Scope | Play *or* generate | **Play + Create + device + training** in one window |
| Format | Funscript-only UI | **Emotion Script** (`.samn`) first — Funscript is share/import only |
| AI | Cloud, bundled weights, or AI-only scripts | **Optional local ONNX** — AI *proposes*, classical tracking *writes* the Emotion Script |
...
**What we deliberately do *not* do:** FunGen clones, silent AI-only scripts,
frameless glassmorphism fashion, or cloud sync. If it is not measured or
not needed, it stays out.

Product format rules: [`docs/EMOTION_SCRIPT.md`](docs/EMOTION_SCRIPT.md).
...
<p align="center">
  <img src="docs/media/gui-generator.png" alt="Create tab — Emotion Script workflow" width="900" />
  <br />
  <em>Create</em> — choose a video, mark where motion lives, build an Emotion Script
</p>

<p align="center">
  <img src="docs/media/gui-playback.png" alt="Play tab — Emotion Script empty state" width="900" />
  <br />
  <em>Play</em> — load an Emotion Script (video optional); heatmap, contact feel, playlist
</p>

<p align="center">
  <img src="docs/media/gui-player-rail.png" alt="Play layout with live vibration and suction meters" width="900" />
  <br />
  Live device meters on the rail — curve and device keep running even without a film
</p>
...
- Import community `.funscript` · **Share for other apps** · **Save Emotion Script**

### Create (Emotion Generator)
- Classical CV first (CSRT / flow / `grid_lk`) — works without any AI install
- Everyday tip tracking + optional Contact vibration / contact marks / tip path
- Optional **local AI ROI** (ONNX), preferred classes, Tf/Tj when you need two regions
- Quality Doctor + human accept/reject → optional learned quality model
- Golden-clip benchmark tab for repeatable comparisons
...
| iPhone / Android | — | **Player only** (no Create) |
...
Latest: **[v0.5.27](https://github.com/funfunpayer/SamNPlayer-site/releases/latest)**  
...
GOOS=windows GOARCH=amd64 wails build -platform windows/amd64 \
  -ldflags "-X github.com/funfunpayer/SamNPlayer/update.Version=v0.5.27"
```

Generator extras need **Python 3.9+** and
`pip install -r generator/requirements.txt`. Optional AI:
`generator/requirements-ai.txt` / train deps via the AI Train tab.
...
```text
  Video ──► [optional ONNX ROI / profile suggestion]
           ...
           Classical tracker (CSRT / flow / grid_lk)
                 │
                 ▼
           Emotion Script (.samn) + Quality Doctor
```

There is **no** second, AI-only path that writes Emotion Scripts by itself.
Details: [`docs/AI_ADAPTER.md`](docs/AI_ADAPTER.md),
...
| Doc | What it is |
|---|---|
| [`docs/EMOTION_SCRIPT.md`](docs/EMOTION_SCRIPT.md) | Product format — Emotion Script / Generator / Funscript share |
| [`docs/AUDIO_WORKFLOW.md`](docs/AUDIO_WORKFLOW.md) | Audio tempo check in Create (post-track) |
...
| [`docs/SAMN_FORMAT.md`](docs/SAMN_FORMAT.md) | Wire format for `.samn` (Neo 2 axes) + Funscript export |
| [`docs/LANGUAGE.md`](docs/LANGUAGE.md) | Product language (English UI/docs) |
| [`docs/WINDOWS_OPENCV.md`](docs/WINDOWS_OPENCV.md) | Windows Go CSRT / portable DLLs |
| [`docs/PRODUCTION_ROADMAP.md`](docs/PRODUCTION_ROADMAP.md) | Production spine G0–G4 |
| [`docs/GENERATE_HEURISTICS.md`](docs/GENERATE_HEURISTICS.md) | Classical Create heuristics + audio workflow |
