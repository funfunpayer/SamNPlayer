...
func FindPython() (string, error) {
	...
	if len(candidates) == 0 {
		return "", fmt.Errorf("generator: kein Python gefunden (python3/python/py im PATH) - bitte Python 3.9+ installieren: https://python.org (bei der Installation \"Add python.exe to PATH\" anhaken)")
	}
...
func CheckDependencies() error {
	...
	return fmt.Errorf(
		"generator: in keinem gefundenen Python sind die benötigten Pakete installiert.\n\n"+
			"Installieren mit GENAU diesem Befehl:\n  \"%s\" -m pip install %s\n\n"+
...
func buildArgs(scriptPath, videoPath, outputPath string, roi ROI, opts Options) []string {
	...
	if opts.ROI2 != nil && opts.ROI2.W > 0 && opts.ROI2.H > 0 {
